...
export function OrdersList({ onSelectOrder, onAddNew }: OrdersListProps) {
  ...
  const filteredOrders = orders.filter((order) =>
    ...
    (order.clientName && order.clientName.toLowerCase().includes(searchText.toLowerCase()))
  );

  const handleDelete = (orderId: string, orderNumber: string) => {
    Alert.alert('Confirmar exclusão', `Deseja excluir a ordem ${orderNumber}?`, [
      ...
      {isLoadingOrders ? (
        ...
        <View style={styles.emptyContainer}>
          <Text style={styles.emptyText}>
            {searchText ? 'Nenhuma ordem encontrada' : 'Você ainda não tem ordens de serviço'}
          </Text>
          {!searchText && (
